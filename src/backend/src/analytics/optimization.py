...
Ported from application/mockup.py (lines 294-317).
"""
from typing import Any
import numpy as np
from .dispatch import run_dispatch
from .gpu_config import compute_capex
...
def battery_sweep(
    ...
    Example:
        >>> curt = np.random.rand(52704) * 10  # 52k timesteps
        >>> price = np.random.rand(52704) * 50 + 30
        >>> results = battery_sweep(
        ...     curt, price, dt_h=1/6, gpu_mw=5.0,
        ...     grid_cap_mw=2.5, btf_cap_mw=2.5,
        ...     btf_price=45.0, curt_price=15.0,
        ...     gpu_type="RTX 5090",
        ...     discount_rate=0.08, project_life=12
        ... )
        >>> len(results)
        10
        >>> results[0]["batt_mwh"]
    ...
    # Fixed battery sizes from mockup (lines 302)
    ...
    for batt_mwh in battery_sizes:
        ...
        # Collect metrics (matching mockup lines 310-316)
        rows.append({
            "batt_mwh": batt_mwh,
